...
/*   Updated: 2019/04/23 16:34:03 by conoel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		sdl_start(SDL_Window **win, SDL_Renderer **ren)
{
	...
	if (!(*win = SDL_CreateWindow("Mes couilles", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 700, 700, SDL_WINDOW_SHOWN)))
	{
...
static void	draw_links(t_node *current, SDL_Renderer *ren, int factor)
{
	...
	while (current->links[i] != NULL)
	{
		...
		SDL_RenderDrawLine(ren, (current->x * factor ) + (SIZE / 2), (current->y * factor) + (SIZE / 2), (current->links[i]->x * factor) + (SIZE / 2), (current->links[i]->y * factor) + (SIZE / 2));
		i++;
	}
}
